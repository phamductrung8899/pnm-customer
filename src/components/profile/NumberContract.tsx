...
export function NumberContract() {
  const { numberContracts } = mockCustomer;

  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">Hợp đồng đăng ký sử dụng đầu số di động</CardTitle>
      </CardHeader>
      <CardContent>
        {numberContracts.length === 0 ? (
          <p className="text-sm text-muted-foreground">Chưa có hợp đồng nào.</p>
        ) : (
          <div className="space-y-2">
            {numberContracts.map((file) => (
